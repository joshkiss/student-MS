...
connector.execute('''CREATE TABLE IF NOT EXISTS 
                  SCHOOL_MANAGEMENT(
...
def add_record():
  ...
  if not name or not email or not contact or not gender or not DOB or not stream:
    mb.showerror('Error!', 'Please fill in all fields in the form')
  try:
    ...
    connector.commit() 
    mb.showinfo('Record Added', f'Record of {name} was added successfully')
    reset_fields()
    display_records()
  except:     
...
def remove_record():
  if not tree.selection():
    mb.showerror('Error!', 'Please select an item to be deleted')
  ...
  connector.commit()
  mb.showinfo('Done', 'Record Deleted')
  display_records()

def view_record():
  global name_strvar, email_strvar, contact_strvar, gender_strvar, dob, stream_strvar
  if not tree.selection():
    mb.showerror('Error!', 'Please select a record to be viewed')
  current_item = tree.focus()
  values = tree.item(current_item)
  selection = values['values']
...
Button(center_frame, text='Delete Record', command=remove_record, font=labelfont, width=15).place(relx=0.1, rely=0.25)
Button(center_frame, text='View Record', command=view_record, font=labelfont, width=15).place(relx=0.1, rely=0.35)
Button(center_frame, text='Reset Fields', command=reset_fields, font=labelfont, width=15).place(relx=0.1, rely=0.45)
Button(center_frame, text='Delete Database', command=reset_form, font=labelfont, width=15).place(relx=0.1, rely=0.55)

# Right frame, displays records
Label(right_frame, text='Student Records', font=headlabelfont, bg='red', fg='LightCyan').pack(side=TOP, fill=X)
